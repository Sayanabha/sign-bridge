// 📄 frontend/src/components/SignToText.jsx  — DAY 4 UPDATE
// New: emits completed sentences via socket.emit('sign-output')
//      so they broadcast to all viewer screens in real time
...
export default function SignToText({ colors, language, socket }) {
  ...
  // ── Commit sentence + broadcast via socket ────────────────────────────────
  const commitSentence = useCallback((text) => {
    const entry = { text, timestamp: Date.now() };
    setSentences(s => [...s, entry]);
    setLastBroadcast(text);

    // Emit to server — server broadcasts to all viewers
    if (socket) {
      socket.emit('sign-output', entry);
      console.log(`[SignToText] Emitted: "${text}"`);
    }
  }, [socket]);

  // ── Handle confirmed sign ─────────────────────────────────────────────────
  ...
  }, [sealWord, commitSentence]);

  const {
    videoRef, canvasRef, isLoading, isReady, error,
    currentSign, currentConfidence, progress,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{
            ...
            background: colors.accent + '22', color: colors.accentGlow || colors.accent,
          ...
          {socket && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              <span style={{ fontSize: '9px', color: colors.accent, fontFamily: 'monospace' }}>BROADCASTING</span>
            </div>
          )}
        </div>
      ...
      {/* Camera */}
      <div style={{ position: 'relative', width: '100%', background: '#000', flexShrink: 0, maxHeight: '220px', overflow: 'hidden' }}>
        ...
        {isLoading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '14px', background: 'rgba(0,0,0,0.85)' }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', border: `2px solid ${colors.border}`, borderTopColor: colors.accent, animation: 'sRotate 0.8s linear infinite' }} />
            <span style={{ fontSize: '12px', color: colors.muted }}>Loading hand tracking...</span>
          </div>
        )}

        {error && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.85)', padding: '20px' }}>
        ...
        {/* Confidence ring */}
        {isReady && currentSign && (
          <div style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            <svg width={52} height={52} style={{ transform: 'rotate(-90deg)' }}>
              <circle cx={26} cy={26} r={R} fill="rgba(0,0,0,0.6)" stroke={colors.border} strokeWidth={3} />
              <circle cx={26} cy={26} r={R} fill="none" stroke={ringColor} strokeWidth={3}
            ...
            <div style={{ background: 'rgba(0,0,0,0.65)', borderRadius: '4px', padding: '2px 6px' }}>
        ...
        {isReady && ambiguityHint && currentConfidence < 0.82 && (
          <div style={{ position: 'absolute', bottom: '8px', left: '10px', right: '60px', background: 'rgba(0,0,0,0.72)', backdropFilter: 'blur(6px)', borderRadius: '6px', padding: '4px 10px', border: '1px solid #F59E0B44' }}>
            <span style={{ fontSize: '9px', color: '#F59E0B', fontFamily: 'monospace' }}>Tip: {ambiguityHint}</span>
          </div>
        )}
      </div>

      {/* Live buffer */}
      <div style={{ padding: '10px 14px', borderTop: `1px solid ${colors.border}`, borderBottom: `1px solid ${colors.border}`, flexShrink: 0, minHeight: '50px', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        {currentWord && (
          <span style={{ fontSize: '22px', fontWeight: 800, color: colors.accent, fontFamily: 'monospace', letterSpacing: '4px', textTransform: 'uppercase' }}>
            {currentWord}
            <span style={{ display: 'inline-block', width: 2, height: 20, background: colors.accent, marginLeft: 4, verticalAlign: 'middle', animation: 'sBlink 0.8s step-end infinite' }} />
          </span>
        )}
        {wordBuffer.map((w, i) => (
          <span key={i} style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '12px', background: colors.live + '22', color: colors.live, border: `1px solid ${colors.live}44`, fontFamily: 'monospace' }}>
            {w}
          </span>
        ))}
        ...
        {sentences.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '10px', color: colors.muted }}>
            ...
            <p style={{ margin: 0, fontSize: '12px', textAlign: 'center', lineHeight: 1.7 }}>
              ...
              {socket ? 'Sent to viewer screens automatically.' : 'Connect socket to broadcast to viewers.'}
            </p>
          </div>
        )}
        {sentences.map((s, i) => (
          <div key={i} style={{
            ...
            border: `1px solid ${i === sentences.length - 1 ? colors.live + '44' : colors.border}`,
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '4px', alignItems: 'center' }}>
              ...
              {socket && (
                <span style={{ fontSize: '9px', color: colors.live, fontFamily: 'monospace' }}>
                  ✓ broadcast
                </span>
              )}
            </div>
